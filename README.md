...
`default_data_config.json` controls splitting, preprocessing, normalisation, training-only augmentation, and DataLoader settings. The experiment seed and target label are applied automatically so that they are recorded consistently. `transform_name` is the deterministic transform used for every split and for normalisation-stat calculation. `train_augmentations` applies only to the training split: `random_resized_crop` replaces that deterministic transform with `RandomResizedCrop(224)` when true; ColorJitter runs on the source image before background subtraction; and `horizontal_flip` controls postprocessing flips before normalisation (`null` disables it; values from 0 to 1 specify the probability). Validation and all test splits always use `transform_name` and remain unaugmented. The default config disables augmentation. `t3_data_config.json` uses a centre crop for evaluation and enables T3-style training augmentations. `ssvtp_color_jitter_settings.json` stores the SSVTP ColorJitter settings (brightness and contrast 0.9–1.1, saturation 0.2, hue 0.05) for reuse in a script.
